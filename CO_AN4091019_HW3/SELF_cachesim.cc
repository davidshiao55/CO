...
void cache_sim_t::init()
{
  ...
  freq = new int[sets * ways];
  for (size_t i = 0; i < sets; i++)
    for (size_t j = 0; j < ways; j++)
      freq[i * sets + j] = 0;

  age = new int[sets * ways];
  for (size_t i = 0; i < sets; i++)
    for (size_t j = 0; j < ways; j++)
      age[i * sets + j] = j;
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
  {
    if (freq[idx * ways + i] < freq[idx * ways + way])
    {
      way = i;
    }
    else if (freq[idx * ways + i] == freq[idx * ways + way])
    {
      if (age[idx * ways + i] < age[idx * ways + way])
        way = i;
    }
  }
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)
{
  ...
  if (likely(hit_way != NULL))
  {
    ...
    for (size_t i = 0; i < ways; i++)
    {
      if (&tags[idx * ways + i] == hit_way)
      {
        freq[idx * ways + i]++;
        age[idx * ways + i] = ways - 1;
      }
      else
      {
        age[idx * ways + i] ? age[idx * ways + i]-- : age[idx * ways + i] = 0;
      }
    }
